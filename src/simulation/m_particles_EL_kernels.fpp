...
contains
                ...
                do di = di_beg, di_end
                    ...
                    if (num_dims == 2) then
                        if (cyl_coord) then
                            ...
                            Vol_loc = dx(cellijk(1))*dy(cellijk(2))*particle_params%charwidth
                        end if
                    else
    ...
    subroutine s_compute_stddsv(cell, volpart, stddsv)
        ...
        stddsv = particle_params%epsilonb*max(chardist, rad)
    ...
    subroutine s_get_char_vol(cellx, celly, cellz, Charvol)
        ...
        if (p > 0) then
            ...
            if (cyl_coord) then
                ...
                Charvol = dx(cellx)*dy(celly)*particle_params%charwidth
            end if
        end if
    ...
    subroutine s_interp_fluid_properties(pos, cell, q_prim_vf, wx, wy, wz, fluid_vel, fluid_rho, fluid_pres)
        ...
        if (particle_params%interpolation_order > 1) then
            do dir = 1, num_dims
    ...
    !     if (abs(val) <= eps) then         val = 0._wp     end if
    ...
    function f_interp_barycentric(pos, cell, field_vf, field_index, wx, wy, wz) result(val)
        ...
        ! Non-finite backstop
        ! With the on-node guard above this should never fire. It remains as a real
        ! catch for a NaN/Inf field value inside the stencil (e.g. an unfilled halo
        ! cell) or a degenerate denominator. val /= val catches NaN; the huge() test
        ! catches +/-Inf. Fall back to the home-cell value, which is an interior
        ! cell for any in-domain particle and therefore finite.
        if (val /= val .or. abs(val) > huge(1._wp)) then
            val = field_vf(field_index)%sf(i, j, k)
        end if

        if (abs(val) <= eps) val = 0._wp
